...
chrome.storage.sync.onChanged.addListener((changes) => {
	if (changes[OPTIONS_KEY_FOR_VSCODE_BUILD] !== undefined) {
		updatePreferredVSCodeBuildDomain(
			changes[OPTIONS_KEY_FOR_VSCODE_BUILD].newValue,
		);
	}
});
...
function redirect(tab) {
	...
	chrome.tabs.update({
		url:
			url !== undefined
				? url.toString()
				: `https://${preferredVSCodeBuildDomain}`,
	});
}
...
function dotComToDotDev(url) {
	try {
		...
		if (
			parsedUrl.hostname.endsWith(GITHUB_COM) ||
			parsedUrl.hostname.endsWith(GITHUB_DEV)
		) {
			parsedUrl.hostname = preferredVSCodeBuildDomain;
			parsedUrl.pathname = `/github${parsedUrl.pathname}`;
			return parsedUrl.toString();
		} else if (
			parsedUrl.hostname.endsWith(AZURE_REPOS) ||
			parsedUrl.hostname.endsWith(AZURE_REPOS_LEGACY)
		) {
			parsedUrl.hostname = preferredVSCodeBuildDomain;
	...
	chrome.storage.local.get(REPO_MAP_LOCAL_STORAGE_KEY, function (storageObj) {
		...
		for (const fullName in repoMap) {
			...
			if (isSingleKeyword) {
				...
				if (
					keywords.filter((keyword) => !inFullName(keyword))
						.length === 0
				) {
					suggestions.push(suggestion);
				}
		...
		if (suggestions.length > 0) {
			...
			isSingleKeyword &&
			input.split("/").filter((segment) => segment.trim() !== "")
				.length === 2
		) {
			...
			defaultSuggestionDescription =
				'<match>No match found. Search GitHub with "' +
				input +
				'"</match>';
...
chrome.tabs.onUpdated.addListener(function (tabId, changeInfo) {
	if (changeInfo.url) {
		...
		chrome.storage.local.get(
			REPO_MAP_LOCAL_STORAGE_KEY,
			function (storageObj) {
				const repoMap = storageObj.repoMap;
				const didUpdateMap = addRepoToMap(
					repoMap,
					owner,
					repoName,
					fullName,
				...
				if (didUpdateMap) {
					console.log(
						"Adding " + fullName + " to repoMap in local storage",
					);

					chrome.storage.local.set({ repoMap: repoMap }, function () {
						console.log(
							"RepoMap updated successfully in local storage",
						);
					});
				}
			},
